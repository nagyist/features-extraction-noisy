import ntpath
from PIL.Image import DecompressionBombWarning, Image
...
def loadImageList(image_paths, img_size=None, crop_size=None, color_mode="rgb", out=None, verbose=True, ignore_IOError=True,
                  ignore_big_images=True, big_images_pixels=None):
    img_list = []
    ...
    for i, im_path in enumerate(image_paths):
        if verbose:
            _progress(i, len(image_paths), status=" - Processing image in label")
        ...
        print ""
        print "img size = {} ".format(img.nbytes)

        img_list.append(img)
    print ""
    print "imglist size = {} ".format(sys.getsizeof(img_list))
    ...
    try:
        ...
        raise ValueError('when img_size and crop_size are None, image in image_paths must have the same shapes.')

    if ignore_big_images:
        warnings.filterwarnings('default') #  restore default warning behaviour
...
def loadImageFolder(path, max_imgs=-1, img_size=None, crop_size=None,
                    color_mode="rgb", extensions=DEFAULT_EXTENSIONS, sort=False, out=None, verbose=False):
    ...
    data = []
    if len(imgFiles) > 0:
        data = loadImageList(imgFiles, img_size, crop_size, color_mode, out, verbose=verbose)
...
class ImageDataset:
    ...
    def loadSingleFolder(self, path, max_imgs=-1, crop_size=None, img_size=None, color_mode="rgb",
    ...
    def loadFolderWithLabels(self, dataset_path, max_img_per_label=-1,
                             ...
                             parallel_threads=0, verbose=True):
        ...
        labelmap = []
        filenames = []
        dataset = np.zeros([0, 3, crop_size[0], crop_size[1]])  #[index,  channels, pixel_y, pixel_x] # TODO: invert crop_size[0] and 1?
        ...
        if parallel_threads>=1:
            ...
            import sys
            if verbose:
                print("ImageDataset.loadFolderWithLabels - sequential mode (1 thread)")
            ...
            for i, l in enumerate(labels):
                ...
                #labelmap_str[l] = i
                if verbose:
                    _progress(i, len(labels), status=" - Processed label: {}".format(l))
                print ""
                ...
                # print "labelset  size = {} ".format(sys.getsizeof(labelset))
                # print "labelmap  size = {} ".format(sys.getsizeof(labelmap))
                # print "filenames size = {} ".format(sys.getsizeof(filenames))
                # print ""
                # print "label l   size = {} ".format(sys.getsizeof(l))
                # print "data  d   size = {} ".format(sys.getsizeof(d))
                # print "fname f   size = {} ".format(sys.getsizeof(fname))
...
def _job(i, labels, dataset_path, max_imgs, crop_size, img_size, color_mode, extensions, sort):

    [d, fname] = loadImageFolder(path=join(dataset_path, labels[i]), max_imgs=max_imgs,
                                 crop_size=crop_size, img_size=img_size,
                                 color_mode=color_mode, extensions=extensions, sort=sort)
    _progress(i, len(labels), status=" - Processed label: {}".format(labels[i]))
    gc.collect()
    return [d, fname]
