...
def main(args):
    ...
    if config.USE_TOY_DATASET:
        ...
        trainset = common.feat_dataset(trainset_name, feat_net)
        validset = common.feat_dataset(config.DATASET + '_valid', feat_net)
    ...
    def for_resnet50():
        ...
        addestra(A, "A", optimizer , 5, callbacks, chk_period=1, loss_in_name=True)

        shallow_path = common.shallow_path("A", trainset_name, feat_net, ext=False)
    ...
    def for_resnet50_HL():
        ...
        optimizer = SGD(lr=0.01, momentum=0.9, decay=1e-6, nesterov=True)
        addestra(HL8K, "HL8K", optimizer, 5, callbacks, chk_period=1, loss_in_name=True)

        shallow_path = common.shallow_path("HL8K", trainset_name, feat_net, ext=False)
        early_stopping = EarlyStopping('val_loss', min_delta=0.001, patience=10, verbose=1)
        reduceLR = ReduceLROnPlateau('val_loss', factor=0.1, patience=4, verbose=1, epsilon=0.0001)
        callbacks = [early_stopping, reduceLR]

        LF = new_model(in_shape, out_shape, hiddens=[Hidden(8000, 0.5)], lf=True, lf_decay=0.03)
        LF.load_weights(shallow_path + '.weights.best.h5', by_name=True)
        optimizer = SGD(lr=0.0001, momentum=0.9, decay=1e-6, nesterov=True)
        addestra(LF, "LF_FT_HL8K", optimizer, epochs=100, callbacks=callbacks, chk_period=1)
