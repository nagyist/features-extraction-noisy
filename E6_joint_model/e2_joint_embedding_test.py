...
def joint_embedding_train(visual_features=cfg_emb.VISUAL_FEATURES_TRAIN,
    ...
    weight_path = path + '.weights.40.h5'

    model = JointEmbedder.load_model(model_path=model_path, weight_path=weight_path)

    # print("\nTest traning: ")
    # map = test_joint_map(visual_features, text_features, class_list, joint_model=model)
    # print("mAP = " + str(map))
    #
    # print("\nTest validation: ")
    # map = test_joint_map(visual_features_valid, text_features, class_list, joint_model=model)
    # print("mAP = " + str(map))
    #
    print("\nTest zero shot test: ")
