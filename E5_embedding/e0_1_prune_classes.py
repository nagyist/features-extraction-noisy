...
def test_for_top_classes(shallow_net, shallow_tester, nb_selected_classes=cfg_emb.PRUNING_KEEP_N_CLASSES,
    ...
    keep_class_list_ordered = keep_class_list[:]
    keep_class_list_ordered.sort()
    if out_classname_txt is not None:
        of = codecs.open(out_classname_txt, 'w', encoding='utf-8')
        for cls in keep_class_list_ordered:
    ...
    prune_class_list.sort()
    return keep_class_list_ordered, prune_class_list
