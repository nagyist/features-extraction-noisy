...
def shallow_finetune_labelflip(feat_net='resnet50', double_seeds=True, outlier_model=False):
    ...
    if feat_net == 'resnet50':
        ...
        ST.train(SNB.H8K().load(10), opt, epochs=100, callbacks=callbacks, chk_period=1)
        ST.train(SNB.H4K().load(10), opt, epochs=100, callbacks=callbacks, chk_period=1)
        ST.train(SNB.A().load(10), opt, epochs=100, callbacks=callbacks, chk_period=1)
